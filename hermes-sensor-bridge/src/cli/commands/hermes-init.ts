...
export async function runHermesInitCommand(args: ParsedArgs): Promise<void> {
  ...
  if (result.alreadyEnabled && !result.configYamlModified && !result.envModified) {
    nextSteps.push(
      "Hermes webhook platform was already enabled — no changes were made.",
    );
  } else {
    if (result.configYamlModified) {
      nextSteps.push(
        `Wrote a managed 'platforms.webhook' block to ${result.configYamlFile}.`,
      );
    }
    if (result.envModified) {
      nextSteps.push(
        `Wrote managed WEBHOOK_* env vars to ${result.hermesEnvFile}.`,
      );
    }
    nextSteps.push(
