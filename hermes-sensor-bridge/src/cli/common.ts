...
export function getPort(args: ParsedArgs): number {
...
export function bridgePackageRoot(): string {
  return fileURLToPath(new URL("../../", import.meta.url));
}

export function resolveSupervisorBin(): string {
...
export async function readRuntimeState(
  ...
  try {
    ...
    if (!healthRes.ok || !listRes.ok) return null;
    return {
...
export async function maybeReloadSupervisor(
  port: number,
  paths: BridgePaths,
...
export async function isSupervisorRunning(paths: BridgePaths): Promise<{
  pid: number | null;
  running: boolean;
}> {
 ...
 */
export async function ensureHermesWebhookEnabled(
  paths: BridgePaths = getBridgePaths(),
  opts: { port?: number; secret?: string } = {},
): Promise<EnsureHermesWebhookResult> {
  const port = opts.port ?? 8644;

  const yamlAlreadyEnabled = await detectWebhookEnabledInConfigYaml(
  ...
  const detectedVia: "config-yaml" | "managed-block" | null = alreadyEnabled
    ? "config-yaml"
    : !configYamlModified && !envModified
      ? "managed-block"
      : null;
  const gatewayRestartRequired =
    (configYamlModified || envModified) && (await isHermesGatewayRunning());
...
async function ensureManagedBlockInConfigYaml(
  configFile: string,
  port: number,
  secret: string,
): Promise<boolean> {
  ...
  if (current.includes(MANAGED_BLOCK_BEGIN)) return false;

  if (hasUnmanagedTopLevelPlatforms(current)) {
    throw new Error(
      `~/.hermes/config.yaml already declares a top-level 'platforms:' block. ` +
...
async function ensureManagedBlockInEnv(
  envFile: string,
  port: number,
  secret: string,
): Promise<boolean> {
 ...
 */
function hasUnmanagedTopLevelPlatforms(yamlText: string): boolean {
  ...
  for (const rawLine of lines) {
    ...
    if (/^platforms\s*:/.test(rawLine)) return true;
  }
  return false;
}

async function detectWebhookEnabledInConfigYaml(configFile: string): Promise<boolean> {
  let text: string;
  try {
...
async function isHermesGatewayRunning(): Promise<boolean> {
