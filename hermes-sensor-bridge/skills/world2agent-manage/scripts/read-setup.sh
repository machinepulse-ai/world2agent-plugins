#!/usr/bin/env bash
# read-setup.sh — install a sensor package and emit its SETUP.md.
#
# Args:    <package>  full npm package name
# Env in:  NPM_DEBUG=1 to leak npm output to stderr
# Stdout:  {"ok":true,"package":"...","package_dir":"...","skill_id":"...",
#           "setup_md_present":bool,"setup_md":"..."}
# Exit:    0 ok / 1 invalid name, npm install failure, etc.
...
if [ "${NPM_DEBUG:-}" = "1" ]; then
  npm install --prefix "$npm_root" --no-audit --no-fund "$pkg" >&2 \
    || out_err "npm install $pkg failed (see stderr)"
else
  log=$(mktemp)
  if ! npm install --prefix "$npm_root" --no-audit --no-fund "$pkg" >"$log" 2>&1; then
    cat "$log" >&2
    rm -f "$log"
    out_err "npm install $pkg failed (set NPM_DEBUG=1 to see full output)"
  fi
  rm -f "$log"
fi

pkg_dir="$npm_root/node_modules/$pkg"
[ -d "$pkg_dir" ] || out_err "$pkg_dir does not exist after npm install — package name may be wrong"
