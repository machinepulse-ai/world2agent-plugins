...
import { getBridgePaths, readManifest, removeSensorEntry, writeManifest, } from "../../supervisor/manifest.js";
import { getPort, maybeReloadSupervisor, printJson, bridgePackageRoot, removePath, runCommand, } from "../common.js";
export async function runRemoveCommand(args) {
...
async function removeSubscriptionFromFile(file, name) {
    try {
        const raw = JSON.parse(await readFile(file, "utf8"));
        const next = stripSubscription(raw, name);
        if (next === raw)
            return;
        await import("../../supervisor/manifest.js").then(({ writeTextAtomic }) => writeTextAtomic(file, JSON.stringify(next, null, 2) + "\n"));
    }
    catch {
        // best effort
    }
}
async function subscriptionStillPresent(file, name) {
    try {
        const raw = JSON.parse(await readFile(file, "utf8"));
...
function stripSubscription(value, name) {
...
function containsSubscription(value, name) {
    if (Array.isArray(value)) {
...
function matchesSubscription(value, name) {
    return (!!value &&
        typeof value === "object" &&
        (value.name === name ||
            value.subscription_name === name));
}
