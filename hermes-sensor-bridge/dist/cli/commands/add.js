...
import { defaultSensorId, ensureBridgeDirs, getBridgePaths, loadOrCreateHmacSecret, readManifest, upsertSensorEntry, writeManifest, } from "../../supervisor/manifest.js";
import { ensureHermesWebhookEnabled, ensurePackageInstalled, getPort, getStringFlag, hasFlag, maybeReloadSupervisor, printJson, runCommand, } from "../common.js";
export async function runAddCommand(args) {
    ...
    const hmacSecret = await loadOrCreateHmacSecret(paths, getStringFlag(args, "hmac-secret"));
    const hermesWebhook = noHermesSubscribe
        ? null
        : await ensureHermesWebhookEnabled(paths, { secret: hmacSecret });
    const { webhookUrl, subscriptionName, subscribeResult } = noHermesSubscribe
        ? {
            webhookUrl: requireString(webhookUrlFlag, "--webhook-url is required with --no-hermes-subscribe"),
...
async function loadConfig(configFile, installed) {
    if (!configFile) {
        const setupPath = String(installed.packageJson.w2a?.setup ?? "SETUP.md");
        throw new Error(`Interactive setup is not implemented; use --config-file <path>. Sensor guidance: ${join(installed.packageRoot, setupPath)}`);
    }
...
async function subscribeWithHermes(sensorId, skillId, hmacSecret) {
...
function parseSubscribeOutput(stdout) {
...
async function writeGenericSkill(hermesSkillsDir, skillId, pkg, installed) {
    const sourceType = String(installed.packageJson.w2a?.source_type ?? pkg);
    const signals = installed.packageJson.w2a?.signals?.join(", ");
