...
export async function startGatewayWatch(options) {
    ...
    const timer = setInterval(() => {
        void checkGatewayPid(options).catch((error) => {
            options.log(`[w2a/gateway-watch] error: ${error instanceof Error ? error.message : String(error)}`);
        });
    }, 10_000);
    timer.unref();
    return () => {
...
async function checkGatewayPid(options) {
