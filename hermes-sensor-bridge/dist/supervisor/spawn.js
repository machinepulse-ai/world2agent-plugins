...
export class SensorSupervisor {
    ...
    runnerBin = fileURLToPath(new URL("../runner/bin.js", import.meta.url));
    constructor(options) {
    ...
    async spawn(entry, restartCount = 0) {
    ...
    async terminate(handle, graceMs = 5_000) {
    ...
    async terminateAll(graceMs = 5_000) {
        this.desiredEntries.clear();
        for (const sensorId of this.restartTimers.keys()) {
            this.clearRestartTimer(sensorId);
        }
    ...
    attachChildStreams(handle) {
        ...
        pipeStream(handle.process.stdout, (line) => {
            ...
            try {
                ...
                this.log(`[w2a/${handle.sensorId}] dropped non-JSON line on stdout: ${truncate(line, 240)}`);
                return;
            }
            void this.deliverSignal(handle, parsed).catch((error) => {
                this.log(`[w2a/${handle.sensorId}] delivery error: ${errorMessage(error)}`);
            });
        });
     ...
     */
    async deliverSignal(handle, signal) {
        ...
        try {
            ...
            this.log(`[w2a/${handle.sensorId}] POST failed for signal ${signalId}: ${errorMessage(error)}`);
        }
    }
    async handleExit(handle, code, signal) {
        handle.lastExitCode = code;
        const current = this.handles.get(handle.sensorId);
        if (current !== handle)
            return;
        this.handles.delete(handle.sensorId);
        this.log(`[w2a/${handle.sensorId}] exited code=${String(code)} signal=${String(signal)}`);
        if (handle.stopping)
            return;
        if (code !== null && NO_RESTART_EXIT_CODES.has(code))
            return;
        ...
        this.log(`[w2a/${handle.sensorId}] scheduling restart in ${delayMs}ms (restart #${nextRestartCount})`);
        const timer = setTimeout(() => {
            this.restartTimers.delete(handle.sensorId);
            void this.spawn(nextEntry, nextRestartCount).catch((error) => {
                this.log(`[w2a/${handle.sensorId}] restart failed: ${errorMessage(error)}`);
            });
        }, delayMs);
        timer.unref();
        this.restartTimers.set(handle.sensorId, timer);
    }
    clearRestartTimer(sensorId) {
 ...
 */
export async function httpPost(url, body, headers, opts) {
    let lastError;
 ...
 */
export function renderPrompt(signal) {
