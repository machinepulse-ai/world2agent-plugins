...
#           "default_sensor_id":"...","setup_md_present":bool,"setup_md":"..."}
...
default_sensor_id=$(package_to_default_sensor_id "$pkg")
...
out_ok "$(jq -nc \
  ...
  --arg default_sensor_id "$default_sensor_id" \
  --arg setup_md "$setup_md" \
  --argjson present "$setup_md_present" \
  '{package:$pkg,package_dir:$pkg_dir,skill_id:$skill_id,default_sensor_id:$default_sensor_id,setup_md_present:$present,setup_md:$setup_md}')"
