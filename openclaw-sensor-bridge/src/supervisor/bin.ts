...
import { resolveOpenClawConnection } from "./openclaw-config.js";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // Resolve gateway URL + hook token + sessionKey prefix from
    // ~/.openclaw/openclaw.json (or env overrides). Fail fast — no point
    // spawning sensors if /hooks/agent isn't reachable.
    const openclaw = await resolveOpenClawConnection();
    log(
      `[w2a/supervisor] openclaw connection: gateway=${openclaw.gatewayUrl} sessionKeyPrefix=${openclaw.defaultSessionKeyPrefix}`,
    );
