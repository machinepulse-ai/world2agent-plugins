...
async function importSensorSpec(packageName) {
    ...
    try {
        ...
        if (isNotFound) {
            console.error(`[world2agent]     ERROR: package "${packageName}" is not installed.\n` +
...
async function main() {
    ...
    const mcp = new Server({ name: channelName, version: "0.1.0-alpha.0" }, {
        capabilities: {
    ...
    const summarizeDiff = (d) => {
        const parts = [];
        if (d.started.length > 0)
            parts.push(`Started: ${d.started.join(", ")}.`);
        if (d.restarted.length > 0)
            parts.push(`Restarted (config changed): ${d.restarted.join(", ")}.`);
        if (d.stopped.length > 0)
            parts.push(`Stopped (removed from config): ${d.stopped.join(", ")}.`);
        ...
        if (d.failed.length > 0)
            parts.push(`Failed to load: ${d.failed.join(", ")}. See stderr for details.`);
        if (parts.length === 0)
            return "No changes. Sensor set is up to date.";
        return parts.join(" ");
    };
    // Tool handlers ────
    //
    ...
    const channelDisabledMessage = () => "Channel notifications are not flowing in this Claude Code session " +
    ...
    if (channelEnabled === null) {
        ...
        setTimeout(() => {
            if (channelEnabled === null) {
                log(`Channel handshake not acknowledged within ${HANDSHAKE_TIMEOUT_MS}ms — ` +
    ...
    if (!clientChannelEnabled) {
        log("Channel disabled (handshake not acknowledged). Skipping sensor " +
        ...
        if (diff.failed.length > 0 && handles.size === 0) {
            ...
            await sendNotification({
                method: "notifications/claude/channel",
                params: {
                    content: "World2Agent plugin loaded, but all configured sensors failed to start (likely missing npm packages). " +
